...
# ---- Linear Model ----
mod1 <- lm(diam ~ age,data = Obs_diam)
# extract coefficients
int <- coef(mod1)[1] # intercept
bla <- coef(mod1)[2] # slope

# ---- Graph ----
ggplot(data = Obs_diam, aes(x = age, y = diam)) +
...
```{r}
#Calcul du NCI
data_NCI <- data |> # data = jeu de donnée initial
  filter(CensusYear == 2010,CodeAlive=TRUE) |> #Filtre sur l'année pour laquelle vous voulez calculer le NCI (si plusieurs année il faudra faire un groupe by année et calculer la moyenne dans la boucle principale)
  select(Plot.x,idTree,Xutm,Yutm,CensusYear,Circ,POM) # Sélection des variables d'intérêt (vous avez pas besoin de Plot.x normalement)
...
geometree <- epure_data_sf |> #Ici epure_data_sf c'est le jeu de donné qui contient uniquement nos arbres dont on regarde la croissance (Pour le NCI on regardait tous les arbres vivants à l'année cible)
  select(idTree,geometry) # on extrait la géométrie et idtree -> geometree (c'est hilarant)
