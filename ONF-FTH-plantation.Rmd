...
### Diamètre (Michaelis-Menten)

```{r}
#| label: fig-diam-real
#| fig-cap: Modèles de croissance en diamètre avec données réelles (29 essences)
#| echo: false
...
Obs_diam_averaged <- Obs_diam %>%
  ...
  summarise(
    diam_mean = mean(diam, na.rm = TRUE),  # Moyenne des diamètres
...
mods_diam <- Obs_diam_averaged %>%
  group_by(essence) %>%
  group_map(~ tryCatch(
    nls(diam_mean ~ (V_max * age) / (K_m + age), data = .x, start = list(V_max = 10, K_m = 0.06),
...
# ---- Graphique of data + prediction ----
ggplot() +
  geom_line(data = Pred_diam, aes(x = age, y = diam, color = essence), linewidth = 1) +
  geom_point(data = Obs_diam_averaged, aes(x = age, y = diam_mean, color = essence), size = 1) +
  theme_minimal() +
  labs(x = "Âge (ans)", y = "Diamètre (cm)", color = "Essence") +
  scale_y_continuous(
...
### Test model diamètre par arbre

```{r}
#| label: fig-diam-debug
#| fig-cap: Debuging figure for diamater model
#| echo: false

# ---- Simplify data to fix NA models bugs ----
Inv_select <- Inv_complet[Inv_complet$Nom_Vernaculaire == "angelique", ]
#Inv_select <- Inv_complet[Inv_complet$Code_Arbre=="MDF01_A_1_1_387",]

# ---- List of NA bug models ----
# mods_diam[sapply(mods_diam, is.null)]
# ---- alimiao ----
# BUG : all DBH are 0
# ---- angelique ----
# BUG : to much 0 at begining
# ---- pin caraibes ----
# BUG : model don't fit with initial state
# ---- bois grage ----
# BUG: at 23 Circonf 0 -> remove data
# ---- cedre sam ----
# BUG : to much 0 at begining
# ---- cœur dehors ----
# BUG : to much 0 at begining
...
# ---- Check number of trees mesear at each time ---
# age_table <- Inv_complet %>%
#   group_by(Nom_Vernaculaire, Age_Plantation_annee) %>%
#   summarise(count = n(), .groups = 'drop')

# ---- Selection des Données de diamètres par essence ----
Obs_diam <- data.frame(
  "diam" = Inv_select$DBHCorr,
  "arbre" = as.factor(Inv_select$Code_Arbre), # as factor
  "age" = Inv_select$Age_Plantation_annee # converted in year
)
...
mods_diam <- Obs_diam %>%
  group_by(arbre) %>%
  group_map(~ tryCatch(
    nls(diam ~ (V_max * age) / (K_m + age), data = .x, start = list(V_max = 40, K_m = 0.06),
        nls.control( maxiter = 500, tol = 1e-5, minFactor = 1e-8)),
    error = function(e) NULL
  ))
names(mods_diam) <- unique(Obs_diam$arbre)

# ---- Prédictions diamètre ----
ages_pred <- 0:60                 # vector of ages
arbre <- unique(Obs_diam$arbre)   # vector of trees
Pred_diam <- expand_grid(age = ages_pred, arbre = arbre) %>%  # dataframe of diam predicted
  group_by(arbre) %>%
  mutate(diam = if (!is.null(mods_diam[[unique(arbre)]]))
           predict(mods_diam[[unique(arbre)]], newdata = data.frame(age=age))
         else NA)

# ---- Graphique of data + prediction ----
ggplot() +
  geom_line(data = Pred_diam, aes(x = age, y = diam, color = arbre), linewidth = 1) +
  geom_point(data = Obs_diam, aes(x = age, y = diam, color = arbre), size = 1) +
  theme_minimal() +
  labs(x = "Âge (ans)", y = "Diamètre (cm)") +
  scale_y_continuous(
    breaks = seq(0, max(Obs_diam$diam, na.rm = TRUE), by = 10),  # Espacement des étiquettes sur l'axe Y
    labels = scales::comma  # Option pour ajouter des séparateurs de milliers
  )+
...
# # ---- Modèle moyen ---- (not already tested)
# library(nlme)
# 
# # modèle mixte non-linéaire
# mod_moyen <- nlme(
#   diam ~ (V_max * age) / (K_m + age),
#   data = Obs_diam,
#   fixed = V_max + K_m ~ 1,              # effets fixes (moyenne globale)
#   random = V_max + K_m ~ 1 | arbre_id,  # effets aléatoires par arbre
#   start = c(V_max = 40, K_m = 5)        # à ajuster
# )
# summary(mod_moyen)

```

### Hauteur (Chapman-Richards)

```{r}
#| label: fig-haut-real
#| fig-cap: Modèles de croissance en hauteur avec données réelles (29 essences)
#| echo: false
...
2.  Moove /pi in DBH in import data

3.  Add data from 2025

4.  Modify volume model without mortality model (sum of volumes)
