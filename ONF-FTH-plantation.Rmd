...
```{r}
#| label: fig-diam-real
#| fig-cap: Modèles de croissance en diamètre avec vraies données (6 essences, 3 parcelles)
#| echo: false
...
Obs_diam <- data.frame(
  "diam" = as.numeric(Inv_complet$Circonférence)/pi,
  "essence" = as.factor(Inv_complet$Nom_Vern),
  "age" = Inv_complet$Age_Plantation_annee
)

# Réduire à un seul point par 'age' et 'essence' en moyennant les duplicats
Obs_diam_simplified <- Obs_diam %>%
...
ggplot() +
  geom_line(data = Pred_diam, aes(x = age, y = diam, color = essence), linewidth = 1) +
  geom_point(data = Obs_diam_simplified, aes(x = age, y = diam, color = essence), size = 1) +
  theme_minimal() +
  labs(x = "Âge (ans)", y = "Diamètre (cm)", color = "Essence") +
  scale_y_continuous(
