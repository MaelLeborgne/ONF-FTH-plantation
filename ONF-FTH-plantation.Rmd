...
# ---- Test statistiques ----
# Effectuer un test wilcoxon pairwise
...
# Extraire la matrice des p-values ajustées
p_values <- pairwise_res$p.value
# Utiliser multcompLetters pour générer les lettres de comparaison
library(multcompView)
cld_res <- multcompLetters(p_values)  # ou ajuster en fonction des résultats spécifiques
...
lm_BAR <- lm(BAR ~ 0 + Age_Plantation_annee + Combinaison:Age_Plantation_annee, data = filtered_data_selection)
...
coeff <- coefficients(lm_BAR)
intercept <- coeff["(Intercept)"][[1]]

# Prediction
slope_summary$slopes <- predict(lm_BAR, newdata = slope_summary)
slope_summary <- slope_summary %>%
  select(-Combinaison,-Age_Plantation_annee) %>%
...
df_lm_pred <- filtered_data_selection %>% 
  ...
  summarise(
    Localisation = unique(Localisation),
    Nom_Vernaculaire = unique(Nom_Vernaculaire),
    Age_Plantation_annee = c(0,20) # two point of prediction
    )

# Prediction on new data
df_lm_pred$BAR <- predict(lm_BAR, newdata = df_lm_pred)

# # ---- Selection ----
# df <- filtered_data_selection[filtered_data_selection$Nom_Vernaculaire=="bagasse",]

# ---- Plots ----
ggplot(data = filtered_data_selection, aes(x = Age_Plantation_annee, y = BAR, color = Nom_Vernaculaire)) +
  geom_point() +
  geom_line(
    data = df_lm_pred, 
    aes(x = Age_Plantation_annee, y = BAR, color = Nom_Vernaculaire)
    ) +
...
```{r}
 ...
 # forcer les intercept à zero
lm_BAR_2 <- lm(BAR ~ 0 + Age_Plantation_annee + Combinaison:Age_Plantation_annee + Combinaison:Code_Arbre, data = filtered_data_selection)
...
M0 <- lm(BAR ~ 1, data = filtered_data_selection)
M1 <- lm(BAR ~ Nom_Vernaculaire, data = filtered_data_selection)
M2 <- lm(BAR ~ Localisation, data = filtered_data_selection)
M12 <- lm(BAR ~ Nom_Vernaculaire + Localisation, data = filtered_data_selection)
