...
df_sp_count <- data_selection %>%
  group_by(Localisation, Nom_Vernaculaire) %>% # for each localisation and specie
  summarise(nb_indiv = n_distinct(Code_Arbre)) %>% # count individuals
  pivot_wider(names_from = Nom_Vernaculaire, values_from = nb_indiv) %>%
  replace_na(list(
    `bagasse` = 0, 
    `angelique` = 0, 
    `yayamadou marecage` = 0
  ))
# Row for Sum
sum_row <- df_sp_count %>%
  group_by() %>%
  summarise(across(2:4, sum, na.rm = TRUE))

# Add the row to table
df_sp_count <- bind_rows(df_sp_count, sum_row) %>%
  mutate(Localisation = ifelse(is.na(Localisation), "Total", Localisation))
...
df_summary_plantation <- left_join(df_sp_count,df_year_plantation,by="Localisation")
...
df <- filtered_data_selection %>%
  filter(Age_Plantation_annee <= 20) %>% # keep age lower than 20
  # keep the oldest inventory
  
  # group_by(Code_Arbre) %>%         # For each individual
  # arrange(Age_Plantation_annee) %>%        # arrange dates by Year trough inventories
  # slice_head(n = 1) %>%                    #!# keep the oldest 
  # ungroup()                           # Enable selection

# ---- Selection of trees alive in the last inventory ----

# ---- Plots ----
ggplot(data = df, aes(x = Nom_Vernaculaire, y = AGB_kg, color = Nom_Vernaculaire)) +
  geom_boxplot() +
  #facet_wrap(~Localisation, scales = "free_y") +
  labs(
    x = "Age (years)",
