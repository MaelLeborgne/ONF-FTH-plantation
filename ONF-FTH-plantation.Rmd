...
$$
Y=\frac{Y_{max}*t}{K_m+t}
$$

$Y$ : La variable de croissance à modéliser (Diamètre ou Hauteur), \
...
Obs_haut_averaged <- Obs_haut %>%
  group_by(age, essence) %>%
  summarise(
    haut_mean = mean(haut, na.rm = TRUE),  # Mean of diameters
    .groups = "drop"  # Pour éviter de conserver l'information de regroupement
  )
...
mods_haut <- Obs_haut_averaged %>%
  group_by(essence) %>%
  group_map(~ tryCatch(
    nls(haut_mean ~ (V_max * age) / (K_m + age), data = .x, start = list(A=22,k=0.05,m=1.2)),
    error = function(e) NULL
  ))
...
Pred_haut <- expand_grid(age = ages_pred, essence = essence) %>%      # dataframe of haut predicted
  group_by(essence) %>%
  mutate(haut_mean = if (!is.null(mods_haut[[unique(essence)]]))
           predict(mods_haut[[unique(essence)]], newdata = data.frame(age=age))
         else NA)

# ---- Graphique of data + prediction : total high ----
ggplot() +
  geom_point(data = Obs_haut_averaged, aes(x = age, y = haut_mean, color = essence), size = 1) +
  geom_line(data = Pred_haut, aes(x = age, y = haut_mean, color = essence), linewidth = 1) +
  theme_minimal() +
  labs(
    x = "Âge (ans)",
    y = "Hauteur (m)",
...
1.  Explication of mickaelis menten model, application for high o trees

2.  Simplify volume model (sum of individual volumes)

3.  Séparer les densité de tige et les surfaces selon les parcelles.

4.  Add data from 2025

5.  Change nls() to brms()

6.  Tout traduire en Anglais
