...
Inv_complet <- Inv_complet[!is.na(Inv_complet$DBHCorr), ]

# ---- Selection des Données de diamètres par essence ----
Obs_diam <- data.frame(
  "diam" = Inv_complet$DBHCorr/pi, # as num divided by pi
...
#   group_by(essence) %>%
#   group_map(~ tryCatch(
#     nls(diam_mean ~ A*(1 - exp(-k*age))^m, data = .x, start = list(A=40,k=0.06,m=1.3)),
#     error = function(e) NULL
#   ))
...
mods_diam <- Obs_diam_averaged %>%
  group_by(essence) %>%
  group_map(~ tryCatch(
    nls(diam_mean ~ (V_max * age) / (K_m + age), data = .x, start = list(V_max = 40, K_m = 0.06)),
    error = function(e) NULL
  ))
names(mods_diam) <- unique(Obs_diam_averaged$essence)
...
# # look on Obesrvation points
# table(Obs_diam_averaged)
# # ckeck data on one tree specie
# Obs_diam_averaged[Obs_diam_averaged$essence=="gonfolo kwali",]
...
1.  Créer des données à T=0 pour l'initialisation des models

2.  Correction Diamètre à 130cm

3.  Question : Modéliser pour chaque arbre et moyenner les coef ?
    ou modéliser la moyenne des arbres ?

4.  Modifier le modèle de volume sans prendre en compte le modèle de mortalité (somme des volumes)
